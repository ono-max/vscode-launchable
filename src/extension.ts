...
return;
		...
		try {
			await asyncExec(`/opt/homebrew/bin/mvn test -Dsurefire.includesFile=/Users/s15236/workspace/launchable-java-example/test.txt`, opts)
		} catch (error) {
			console.error(error);
			return;
		}
		...
		try {
			const {stdout} = await asyncExec(`${pythonPath} -m launchable record tests maven target/surefire-reports/TEST-example.*.xml`, opts)
			const found = stdout.match(/(https:\/\/app.launchableinc.com\/organizations.*\/(.*))\sto/)
			if (found) {
				const result = new LaunchableTreeItem(
					"Result",
					{}
				)
				result.children = [
